...
func GetorgusersHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/api/organizations/%s/users%s", cfg.BaseURL, username, queryString)
...
func CreateGetorgusersTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_api_organizations_username_users",
		mcp.WithDescription("Organization's users"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    GetorgusersHandler(cfg),
	}
}
