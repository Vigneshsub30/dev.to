...
func GetorgarticlesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/api/organizations/%s/articles%s", cfg.BaseURL, username, queryString)
...
func CreateGetorgarticlesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_api_organizations_username_articles",
		mcp.WithDescription("Organization's Articles"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    GetorgarticlesHandler(cfg),
	}
}
