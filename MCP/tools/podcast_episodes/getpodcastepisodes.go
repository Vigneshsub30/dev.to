...
func GetpodcastepisodesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/api/podcast_episodes%s", cfg.BaseURL, queryString)
		...
		var result []PodcastEpisodeIndex
...
func CreateGetpodcastepisodesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_api_podcast_episodes",
		mcp.WithDescription("Podcast Episodes"),
		...
		mcp.WithString("username", mcp.Description("Using this parameter will retrieve episodes belonging to a specific podcast.")),
	)
