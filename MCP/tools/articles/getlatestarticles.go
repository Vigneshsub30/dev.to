...
func GetlatestarticlesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/api/articles/latest%s", cfg.BaseURL, queryString)
...
func CreateGetlatestarticlesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_api_articles_latest",
		mcp.WithDescription("Published articles sorted by published date"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    GetlatestarticlesHandler(cfg),
	}
}
