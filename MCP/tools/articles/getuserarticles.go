...
func GetuserarticlesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/api/articles/me%s", cfg.BaseURL, queryString)
...
func CreateGetuserarticlesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_api_articles_me",
		mcp.WithDescription("User's articles"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    GetuserarticlesHandler(cfg),
	}
}
